...
void ThreadPool::worker(void *arg)
{
    ...
    while (true)
    {
        ...
        while (pool->taskQ.empty() && !pool->shutdown)
        {
            ...
            if (pool->exitNum > 0)
            {
                ...
                if (pool->liveNum > pool->minNum)
                {
                    //存活线程数减一
                    pool->liveNum--;
                    cout << "threadid: " << std::this_thread::get_id() << "杀死线程 exit......" << endl;
        ...
        if (pool->shutdown)
        {
            cout << "threadid: " << std::this_thread::get_id() << "线程池 exit......" << endl;
            return;
        }
